...
struct SymbolTableEntry{
    ...
    int firstIndexLen; // aleary * 4, the reg that stores the int for entry
    int secondIndexLen; // offset
};
...
enum DataType findTypeInMethodTable(char *ID); // used in p4
int getIndexOfArg(char *ID, struct ASTNode* node); // use in p5
#endif
