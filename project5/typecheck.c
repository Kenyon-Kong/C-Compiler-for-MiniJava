...
void create_Table(struct ASTNode* node) {
    ...
    table_node->arglist = malloc(sizeof(struct ArgNode));
    table_node->arglist->num_args = 0;
...
void firstTraversal(struct ASTNode* node) {
    ...
    if (node -> newScope == true) {
        // create a new Table
        create_Table(node);
        if(node->node_type == NODETYPE_STMT) {
            node->offsetTable = node->parentNode->offsetTable;
        } else {
            node->offsetTable = malloc(sizeof(struct OffsetTable));
            node->offsetTable->regCount = 0;
        }
        if (node->node_type == NODETYPE_MAINCLASS) {
            ...
            node->offsetTable->regCount = entry->num_args; // initialize the regCounter for each method ////
            num_methods++;
            // printf("ADD method: %s\n", methodName);
        } else if (node->node_type == NODETYPE_MAINMETHOD) {
            addToTable(node->data.value.string_value, DATATYPE_STR, node->table, 1);
            node->offsetTable->regCount = 2;
        } else {
            node->table->arglist = node->table->parentNode->arglist;
            node->table->arglist->num_args = node->table->arglist->num_args;
        }
        ...
        if (node->node_type == NODETYPE_LEFTVAL) {
            ...
            node->table->arglist->ID[node->table->arglist->num_args] = ID;
            node->table->arglist->num_args++;
...
enum DataType findTypeInMethodTable(char *ID) { // find return type of a method in method table
    for (int i = 0; i < num_methods; i++) {
        if (strcmp(method_table[i]->methodName, ID) == 0) {
...
int getIndexOfArg(char *ID, struct ASTNode* node) {
